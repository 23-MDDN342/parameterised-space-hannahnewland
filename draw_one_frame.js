...
//         ellipse(TravelEllipse, 10, HowManySquares*10);
// 		ellipse(SpacingAmount-15, TravelEllipse, HowManySquares*10);
// 		}
...
// }
	...
	for(let across =0; across < w / SpacingAmount-1; across++){
		...
		DrawSnakes(cur_frac, SpacingAmount,HowManySquares);
	}

function DrawSnakes(cur_frac, Spacing,ManySquares){
	
	let animateCircle = map(cur_frac, 0, 1, 0, Spacing);
	push();
	...
	for(let ii = 0; ii < Spacing; ii++){
				...
				stroke(random(colors));
				let TravelEllipse = animateCircle - (ii*1);
				console.log(TravelEllipse)
		        ellipse(TravelEllipse, 10, ManySquares*10);
				ellipse(Spacing-15, TravelEllipse, ManySquares*10);
				}
    pop();
